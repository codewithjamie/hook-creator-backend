...
/**
 * All AI prompts live here — single source of truth.
 * Porting the OpenEdge hook prompt with improvements.
 */
@Injectable()
export class PromptService {
  /**
   * Build the hook selection prompt for Claude.
   *
   * Key improvements over original Python version:
   *  - Weighted scoring rubric embedded in prompt
   *  - Explicit sentence boundary enforcement (start + end)
   *  - Bridge sentence requirements clarified
...
- The JSON must be parseable — no trailing commas, no comments`;
  ...
  /**
   * System prompt for Claude when processing hooks.
   */
